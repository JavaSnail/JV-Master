...
- 锁问题的关键在于<font color=red>进程操作的互斥关系</font>，例如多个进程同时修改账户的余额，如果没有互斥关系则会导致该账户的余额不正确。
- 而事务问题的关键则在于事务涉及的<font color=red>一系列操作需要满足 ACID 特性</font>，例如要满足原子性操作则需要这些操作要么都执行，要么都不执行。
